...
query_2 = """select name, sum(views) as views from views_table, authors where
            views_table.author = authors.id group by author, authors.name
            order by views desc;"""
query_3 = """select to_char(error_table.date, 'FMMonth dd, yyyy'),
            (error::real/total*100)::decimal(4, 2) from error_table,
            total_table where error_table.date = total_table.date and
            (error::float/total) >= 0.01;"""


# Create view tables:
view_1 = """create view views_table as select title, views, author
...
view_2 = """create view error_table as select date(time) as date,
            count(*) as error from log where status like '4%'
            group by date(time), status order by date(time);"""
view_3 = """create view total_table as select date(time) as date,
...
def view_table_function(query):
    """Commit queries use to create to delete a view table
        form the 'database'."""
...
# most page views? Present this as a sorted list with the most popular
# author at the top.
